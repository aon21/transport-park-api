...
class OrderResource
{
    ...
    public static function fromEntity(Order $order): self
    {
        ...
        $resource->truckId = $order->getTruck() 
            ? $order->getTruck()->getId()->toRfc4122() 
            : null;
        $resource->trailerId = $order->getTrailer() 
            ? $order->getTrailer()->getId()->toRfc4122() 
            : null;
        $resource->fleetSetId = $order->getFleetSet() 
            ? $order->getFleetSet()->getId()->toRfc4122() 
            : null;
        ...
        $resource->endDate = $order->getEndDate() 
            ? $order->getEndDate()->format('Y-m-d\TH:i:s\Z') 
            : null;
